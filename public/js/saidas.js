let filtroSaidasState = { month: hoje.getUTCMonth(), year: hoje.getUTCFullYear(), category: 'todas', status: 'todos', perfil: 'todos' };
...
function renderizarAbaSaidas() {
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
      <div class="md:col-span-1 bg-card-bg text-primary p-6 rounded-xl shadow-lg border border-border self-start">
        ...
        <form id="formSaida" class="space-y-4">
          ...
          <div><label for="categoriaSaida" class="block text-sm font-medium text-subtle-text mb-1">Categoria</label>
            <select id="categoriaSaida" class="${selectClasses}">${CATEGORIAS_SAIDA.map(c => `<option>${c}</option>`).join('')}</select></div>
          ...
          <div class="flex items-center"><input id="isParcelado" type="checkbox" class="item-checkbox"><label for="isParcelado" class="ml-2 block text-sm text-subtle-text">Parcelado</label></div>
          ...
          <div><label for="perfilSaida" class="block text-sm font-medium text-subtle-text mb-1">Perfil</label>
            <select id="perfilSaida" class="${selectClasses}">${getPerfis().map(p => `<option value="${p}">${p || 'Geral'}</option>`).join('')}</select></div>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
          ...
          <div class="flex flex-wrap items-center justify-start sm:justify-end gap-2" id="filtrosSaidas">
            ...
            <select id="filtroCategoriaSaidas" class="${filterSelectClasses}">
              ...
              ${CATEGORIAS_SAIDA.map(cat => `<option value="${cat}" ${filtroSaidasState.category === cat ? 'selected' : ''}>${cat}</option>`).join('')}
            </select>
            ...
            <select id="filtroPerfilSaidas" class="${filterSelectClasses}">
              <option value="todos" ${filtroSaidasState.perfil === 'todos' ? 'selected' : ''}>Todos</option>
              ${getPerfis().map(p => `<option value="${p}" ${filtroSaidasState.perfil === p ? 'selected' : ''}>${p || 'Geral'}</option>`).join('')}
            </select>
          </div>
        </div>
  ...
  document.getElementById('isParcelado').addEventListener('change', (e) => {
    document.getElementById('parcelasContainer').classList.toggle('hidden', !e.target.checked);
  });
  ...
  document.getElementById('formSaida').addEventListener('submit', async (e) => {
    ...
    try {
      await adicionarSaida({
        ...
        numParcelas: isParcelado ? form.numParcelas.value : 1,
        isCustoNegocio: form.isCustoNegocio.checked,
        status: form.statusSaida.value,
        perfil: form.perfilSaida.value,
      });
    ...
    document.getElementById('isParcelado').checked = false;
  ...
  document.getElementById('filtroPerfilSaidas').addEventListener('change', e => {
    filtroSaidasState.perfil = e.target.value;
    renderizarListaSaidas();
  });
...
function renderizarListaSaidas() {
  ...
  if (!listaEl) return;
  const { month, year, category, status, perfil } = filtroSaidasState;
  ...
  filtradas.sort((a, b) => new Date(b.data) - new Date(a.data)).forEach(s => {
    ...
    html += `<li class="flex justify-between items-center p-3 rounded-md border border-border bg-slate-900/30 ${isDizimo ? 'opacity-60' : 'hover:border-secondary/50'}">
      <div class="flex items-center flex-grow gap-4">
        ...
        <div><p class="font-semibold text-light-text">${s.descricao}</p><p class="text-sm text-subtle-text">${s.categoria} - ${formatarData(s.data)}${s.perfil ? ` · ${s.perfil}` : ''}${s.numParcelas > 1 ? ` · <span class="text-warning">${s.numParcelas}x</span>` : ''}</p></div>
      </div>
...
function mostrarModalEditarSaida(id) {
  ...
  const content = `<div class="bg-card-bg text-primary rounded-lg shadow-xl w-full max-w-md m-4 border border-border">
    ...
    <form id="formEditSaida" class="p-6 space-y-4">
      ...
      <div><label class="block text-sm font-medium text-subtle-text mb-1">Categoria</label><select name="categoria" class="${selectClasses}">${CATEGORIAS_SAIDA.map(c => `<option ${saida.categoria === c ? 'selected' : ''}>${c}</option>`).join('')}</select></div>
      ...
      <div class="flex items-center"><input id="editIsParcelado" type="checkbox" name="isParcelado" ${saida.numParcelas > 1 ? 'checked' : ''} class="item-checkbox"><label for="editIsParcelado" class="ml-2 block text-sm text-subtle-text">Parcelado</label></div>
      <div id="editParcelasContainer" class="${saida.numParcelas > 1 ? '' : 'hidden'}"><label class="block text-sm font-medium text-subtle-text mb-1">Nº de Parcelas</label><input type="number" name="numParcelas" class="${inputClasses}" value="${saida.numParcelas || 1}" min="1"></div>
      <div><label class="block text-sm font-medium text-subtle-text mb-1">Perfil</label><select name="perfil" class="${selectClasses}">${getPerfis().map(p => `<option value="${p}" ${(saida.perfil || '') === p ? 'selected' : ''}>${p || 'Geral'}</option>`).join('')}</select></div>
  ...
  document.getElementById('editIsParcelado').addEventListener('change', (e) => {
    document.getElementById('editParcelasContainer').classList.toggle('hidden', !e.target.checked);
  });
  document.getElementById('formEditSaida').addEventListener('submit', async (e) => {
    ...
    if (isNaN(valor) || valor <= 0) { showInfoModal('Valor inválido!'); return; }
    try {
      await editarItem('saidas', form.id.value, {
        ...
        numParcelas: form.isParcelado.checked ? form.numParcelas.value : 1
      });
